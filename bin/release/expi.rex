...
if n>0 then return
repet = 0
if substr(curline.3,lap+1,length(RepeatChars)) = RepeatChars then repet=1
else if substr(curline.3,lap+1,length(RepeatCharsNL)) = RepeatCharsNL then repet=2
":"fil 
if fil = lal then do
  "extract /curline" 
  select
  when repet = 0 then do
     curline.3 = substr(curline.3,1,fip-1)||substr(curline.3,fip+1,lap-fip-1)||substr(curline.3,lap+1)
     "R" curline.3
  end 
  when repet = 1 then do
     curline.3 = substr(curline.3,1,fip-1)||substr(curline.3,fip+1,lap-fip-1)||substr(curline.3,fip)
     "R" curline.3
  end 
  when repet = 2 then do
     l = substr(curline.3,1,fip-1)||substr(curline.3,fip+1,lap-fip-1)                 
     "R" l
     l = substr(curline.3,fip)
     "I" l
  end    
  end    
end
  ...
  when repet = 1 then do
    ...
    "extract /curline" 
    lalln = curline.3
    l = substr(curline.3,1,lap-1) || substr(filln,fip-1)
    "R" l
    do ll=fil+1 to lal-1
      ":"ll 
      "extract /curline" 
      ":"(ll + (lal - fil - 1))
      "I" curline.3
